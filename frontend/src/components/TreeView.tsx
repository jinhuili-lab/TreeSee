...
export default function TreeView() {
  ...
  const { tree, search, colorColumn, setSelected } = useStore()
  ...
  useEffect(()=>{
    ...
    const matched=new Set(nodes.filter((n)=>n.data.is_leaf&&n.data.name.includes(search)).map((n)=>n))
    const pathSet=new Set<any>(); matched.forEach((n)=>{let c:any=n; while(c){pathSet.add(c); c=c.parent}})
    g.selectAll('path.link').data(links).enter().append('path').attr('class','link').attr('fill','none').attr('stroke',(d)=>pathSet.has(d.target)?'#dc2626':'#64748b').attr('d',d3.linkHorizontal<any,any>().x((d:any)=>d.y+20).y((d:any)=>d.x+20) as any)
    g.selectAll('circle.node').data(nodes).enter().append('circle').attr('cx',(d)=>d.y+20).attr('cy',(d)=>d.x+20).attr('r',4).attr('fill',(d)=>{if(!d.data.is_leaf) return '#0f172a'; const v=colorColumn&&d.data.metadata?String(d.data.metadata[colorColumn] ?? ''):''; return v?String(colorScale(v)):'#334155'}).on('click',(_,d)=>{ if(!d.data.is_leaf){ d.data.collapsed=!d.data.collapsed; } setSelected(d.data); })
  ...
  },[tree,search,colorColumn,setSelected,colorScale])
