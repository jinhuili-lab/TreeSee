...
export default function TreeView() {
  ...
  const { rawTree: tree, search, colorColumn, setSelected, toggleCollapse, manualNodeColors } = useStore()
  ...
  useEffect(() => {
    ...
    g.selectAll('circle.node').data(nodes).enter().append('circle').attr('cx', (d) => d.y + 20).attr('cy', (d) => d.x + 20).attr('r', 4).attr('fill', (d) => {
      const manual = manualNodeColors[d.data.id]
  ...
  }, [tree, search, colorColumn, setSelected, toggleCollapse, colorScale, manualNodeColors])
