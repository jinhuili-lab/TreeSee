...
export default function TreeView() {
  ...
  const { tree, view, setSelected, toggleCollapsed, setColorColumn } = useStore()
  ...
  useEffect(() => {
    ...
    const matched = new Set(nodes.filter((n) => n.data.is_leaf && n.data.name.includes(view.search)).map((n) => n))
    ...
    g.selectAll('circle.node').data(nodes).enter().append('circle').attr('cx', (d) => d.y + 20).attr('cy', (d) => d.x + 20).attr('r', 4)
      .attr('fill', (d) => {
        if (!d.data.is_leaf) return '#0f172a'
        const v = view.colorColumn && d.data.metadata ? String(d.data.metadata[view.colorColumn] ?? '') : ''
        return v ? String(colorScale(v)) : '#334155'
      })
      .on('click', (_, d) => {
        if (!d.data.is_leaf) toggleCollapsed(d.data.id)
        setSelected(d.data)
      })

    g.selectAll('text.label').data(nodes.filter((n) => n.data.is_leaf)).enter().append('text')
      .attr('x', (d) => d.y + 28).attr('y', (d) => d.x + 24).attr('font-size', 12)
      .attr('fill', (d) => (pathSet.has(d) ? '#b91c1c' : '#0f172a')).text((d) => d.data.name)
  }, [tree, view, setSelected, toggleCollapsed, colorScale])
  ...
  return <div className='h-full w-full'>
    <div className='p-2 text-sm flex items-center gap-2'>
      ...
      <select className='border ml-2' value={view.colorColumn} onChange={(e) => setColorColumn(e.target.value)}>
        <option value=''>None</option>{cols.map((c) => <option key={c}>{c}</option>)}
      </select>
    </div>
    <svg ref={svgRef} className='w-full h-[calc(100%-40px)] bg-white border' />
  </div>
}
