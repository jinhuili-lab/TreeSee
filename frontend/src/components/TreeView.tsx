...
export default function TreeView() {
  ...
  useEffect(() => {
    ...
    if (layoutMode === 'radial' || layoutMode === 'circle') {
      ...
      g.selectAll('path.link').data(links).enter().append('path').attr('fill', 'none').attr('stroke', (d: any) => manualBranchColors[d.target.data.id] || '#64748b').attr('d', d3.linkRadial<any, any>().angle((d: any) => d.x).radius((d: any) => branchLengthMode === 'topology_only' ? d.depth * 80 : d.y) as any)
      g.selectAll('circle.node').data(nodes).enter().append('circle').attr('cx', (d) => radialPoint(d.x, branchLengthMode === 'topology_only' ? d.depth * 80 : d.y)[0]).attr('cy', (d) => radialPoint(d.x, branchLengthMode === 'topology_only' ? d.depth * 80 : d.y)[1]).attr('r', 4).attr('fill', (d) => manualNodeColors[d.data.id] || '#334155').on('click', (_, d) => { if (!d.data.is_leaf) toggleCollapse(d.data.id); setSelected(d.data) })
      return
    }
    ...
    const linkGen = layoutMode === 'straight' ? d3.linkHorizontal<any, any>().x((d: any) => (branchLengthMode === 'topology_only' ? d.depth * 140 : d.y) + 20).y((d: any) => d.x + 20)
      : d3.linkHorizontal<any, any>().x((d: any) => (branchLengthMode === 'topology_only' ? d.depth * 180 : d.y) + 20).y((d: any) => d.x + 20)
    ...
    g.selectAll('circle.node').data(nodes).enter().append('circle').attr('cx', (d) => (branchLengthMode === 'topology_only' ? d.depth * (layoutMode === 'straight' ? 140 : 180) : d.y) + 20).attr('cy', (d) => d.x + 20).attr('r', 4).attr('fill', (d) => {
    ...
    g.selectAll('text.label').data(nodes.filter((n) => n.data.is_leaf)).enter().append('text').attr('x', (d) => (branchLengthMode === 'topology_only' ? d.depth * (layoutMode === 'straight' ? 140 : 180) : d.y) + 28).attr('y', (d) => d.x + 24).text((d) => d.data.name).attr('font-size', 12).attr('fill', (d) => (pathSet.has(d) ? '#b91c1c' : '#0f172a'))
